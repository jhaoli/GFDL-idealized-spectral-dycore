...
'$Id: steady_state.F90,v 19.0 2012/01/06 20:35:33 fms Exp $'
...
namelist / steady_state_nml / n0, U0, nt, lapse, T0, lonc, latc, deltaT

contains

!=========================================================================================================================

subroutine steady_state(sea_level_press, triang_trunc, &
...
#ifdef INTERNAL_FILE_NML
    read (input_nml_file, nml=steady_state_nml, iostat=io)
    ierr = check_nml_error(io, 'steady_state_nml')
#else
    unit = open_namelist_file()
    ierr=1
    do while (ierr /= 0)
      read(unit, nml=steady_state_nml, iostat=io, end=20)
      ierr = check_nml_error (io, 'steady_state_nml')
    enddo
20  call close_file (unit)
#endif
call write_version_number(version, tagname)
if(mpp_pe() == mpp_root_pe()) write (stdlog(), nml=steady_state_nml)
